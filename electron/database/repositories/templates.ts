...
export function getTemplateById(id: string): Template | null {
  return queryOne('SELECT * FROM templates WHERE id = ?', [id]) as Template | null
}
...
export function createTemplate(data: {
  ...
  run(
    'INSERT INTO templates (id, name, description, label_width, label_height, unit, dpi, printer_type, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [id, data.name, data.description || null, data.label_width, data.label_height, data.unit || 'mm', data.dpi || 300, data.printer_type || null, data.created_by]
  )
...
export function updateTemplate(
  ...
  run(`UPDATE templates SET ${setClauses.join(', ')} WHERE id = ?`, values)
  return getTemplateById(id)
}

export function deleteTemplate(id: string): boolean {
  return runDelete('DELETE FROM templates WHERE id = ?', [id])
}
...
export function archiveTemplate(id: string): Template | null {
  run("UPDATE templates SET status = 'Archived', updated_at = datetime('now') WHERE id = ?", [id])
  return getTemplateById(id)
}

export function updateTemplateStatus(id: string, status: string): Template | null {
  run("UPDATE templates SET status = ?, updated_at = datetime('now') WHERE id = ?", [status, id])
  return getTemplateById(id)
}

export function setTemplateCurrentVersion(id: string, versionId: string): void {
  run("UPDATE templates SET current_version_id = ?, updated_at = datetime('now') WHERE id = ?", [versionId, id])
}
...
export function exportTemplate(id: string): any {
  ...
  const versions = query('SELECT * FROM template_versions WHERE template_id = ?', [id])
  const dataSources = query('SELECT * FROM template_data_sources WHERE template_id = ?', [id])
